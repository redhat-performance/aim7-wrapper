...
cpu_set_t cpuset[MAX_NUMA];

static void
obtain_nums_cpu_binding()
{
	FILE *fd;
	char buffer[1024];
	char *ptr;
	...
	system("numactl --hardware | grep cpu > /tmp/numa_spread");
	
	fd = fopen("/tmp/numa_spread", "r");
	while (fgets(buffer, 1024, fd)) {
		CPU_ZERO(&cpuset[numa_nodes]);
		ptr = strchr(buffer, ':');
		ptr = &ptr[2];
		while(ptr) {
			CPU_SET(atoi(ptr), &cpuset[numa_nodes]);
			ptr = strchr(ptr, ' ');
			if (ptr)
				ptr++;
		}
     ...
     char **argv)
{
	...
	while (--argc > 0) {		/* scan the command line */
		if ((*++argv)[0] == '-') {	/* look at first char of next */
			...
			case 'b':
				numa_bind = atol(&(*argv)[2]);	
				printf("Binding to node %d\n", numa_bind);
			break;
			case 'd':
	...
	if (numa_bind != -1) {
		obtain_nums_cpu_binding();
		/* Bind ourselves to numa node 0 */
		if (sched_setaffinity(getpid(), sizeof(cpu_set_t), &cpuset[numa_bind])) {
...
#if 0
			/* FPR add perf here */
			if (strcmp(getenv("DO_PERF_PROBE"), "") != 0) {
				char perfstr[MAXSTRLEN];
				(void)printf("\nFPR Starting perf\n");
				sprintf(perfstr, "/usr/bin/perf -ag --output=perf.data.%d sleep 60000 &",
		    		1000);
				system(perfstr);
			}
#endif
...
#if 0
			(void)printf("\nFPR Stopping perf\n");
			system("/usr/bin/pkill sleep");
			(void)printf("\nFPR Waiting for perf\n");
			(void)wait(NULL);
			(void)printf("\nFPR Done waiting for perf\n");
#endif
			/*
			 * Are we done? 
			 */
...
get_misc_information()
{
	...
	for (j = 0; j < iters; j++) {	/* for each iteration */
		...
		if (use_file) {
			...
			prompt_read(&minusers[j], 1, 1000000,	/* minimum users ELEVATED BY BMARSON */
			...
			if (answer == 1) {
				...
				termination[j] = NOCROSSOVER;
				prompt_read(&maxusers[j], minusers[j], 1000000,	/* maximum users was 10000 */
					    "Maximum number of operation loads to simulate");
			}
			prompt_read(&incr[j], 1, 50000,	/* increment was 100 */
		...
		for (j = 0; j < numdir; j++) {	/* loop through directories */
			...
			while (k--) {	/* while not done */
				if (write(fd1, file_buffer, sizeof file_buffer) != sizeof file_buffer) {	/* write it */
					perror(fn1);
					(void)fprintf(stderr, "disk_create_all_files: cannot write %s\n", fn1);	/* error message */
 ...
 */
					...
					if (stat(buf, &stbuf) < 0) {	/* tar failed */
						...
						if (stat(buf, &stbuf) < 0) {	/* tar failed */
							perror(dkarr[j]);
							fprintf(stderr, "disk_create_all_files(): tar failed %s/fakeh\n", dkarr[j]);	/* tell user */
							return (-1);
						}
					}
				}	/* end of if stat */
